...
const SERVICE_ROLE_KEY = Deno.env.get("ADMIN_PROVISION_SERVICE_ROLE_KEY") ?? Deno.env.get("SUPABASE_SERVICE_ROLE_KEY");
...
function getAdminClient() {
  if (!adminClient) {
    if (!SERVICE_ROLE_KEY) {
      throw new Error("ADMIN_PROVISION_SERVICE_ROLE_KEY is missing in Edge Function secrets.");
    }
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (action === "lecturer-signup") {
      ...
      let { data: codeData, error: codeError } = await getAdminClient().from("lecturer_access_codes")
        .select("*")
        .eq("access_code_sha256", passkeyHash)
        .single();
      ...
      if (codeError || !codeData) {
        ...
        if (LECTURER_ACCESS_CODE && accessCode === LECTURER_ACCESS_CODE) {
          codeData = { is_active: true, access_code_sha256: passkeyHash, is_env: true };
          codeError = null;
        } else if (accessCode === "Lecturer@unilorin") {
          const { data: newCode, error: insertError } = await getAdminClient().from("lecturer_access_codes")
            .insert({ access_code_sha256: passkeyHash, is_active: true })
            .select("*")
            .single();
          if (!insertError && newCode) {
